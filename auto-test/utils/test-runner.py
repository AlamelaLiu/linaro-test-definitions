...
try:
    ...
    print('Python2: pip install -r ${REPO_PATH}/auto-test/utils/requirements.txt')
    print('Python3: pip3 install -r ${REPO_PATH}/auto-test/utils/requirements.txt')
    sys.exit(1)
...
class TestPlan(object):
    ...
    def test_list(self, kind="auto-test"):
        if self.test_def:
...
class RemoteTestRun(AutomatedTestRun):
    def copy_to_target(self):
        ...
        run_command(
            'tar -caf %s run.sh uuid auto-test/lib auto-test/bin auto-test/utils %s' %
...
def get_args():
    ...
    parser.add_argument('-r', '--test_def_params', default={}, dest='test_def_params',
                        ...
                        '''))
    parser.add_argument('-k', '--kind', default="auto-test", dest='kind',
                        choices=['auto-test', 'manual'],
                        help=textwrap.dedent('''\
                        Selects type of tests to be executed from the test plan.
                        Possible options: auto-test, manual
                        '''))
